...
import { HomeDesktopDashboard } from '../components/home/HomeDesktopDashboard';
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center justify-between gap-2">
              ...
              <div className="text-right shrink-0">
                ...
                <p className="font-bold text-primary text-lg">${provider.price.toLocaleString()}</p>
              </div>
            </div>
          </div>
        </div>
...
function SelectedProviderSheet({
  provider,
  onClose,
  onWalkerClick,
}: {
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-2 text-sm mb-2">
            ...
            <span className="text-muted-foreground">{provider.distance} km</span>
          </div>
          <WalkerAvailabilityBadge walker={provider} size="md" />
        </div>
        <Button
          size="sm"
          ...
          onClick={(e) => {
            e.stopPropagation();
            onWalkerClick(provider);
          }}
        >
          {provider.available ? 'Reservar' : 'Agendar'}
        </Button>
      </div>
    </motion.div>
  );
...
function DesktopSelectedProviderCard({
  provider,
  onClose,
  onWalkerClick,
}: {
  ...
  return (
    <motion.div
      ...
      className="absolute bottom-6 right-6 max-w-sm w-full bg-card rounded-2xl shadow-2xl border border-border p-4 z-30"
...
export const HomeScreen: React.FC<HomeScreenProps> = ({ onWalkerClick }) => {
  ...
  const categorySubtitle = `${filteredProviders.length} resultado${
  ...
  return (
    <div className="h-full flex flex-col bg-background-secondary overflow-hidden">
      <HomeCategoryTabs activeCategory={category} onChange={setCategory} />
      ...
      <div className="hidden md:flex flex-1 min-h-0 overflow-hidden relative">
        <HomeDesktopDashboard
          category={category}
          categoryTitle={categoryMeta.resultsTitle}
          ...
          onProviderClick={onWalkerClick}
          isQuickFilterActive={isQuickFilterActive}
          onQuickFilterToggle={handleQuickFilterToggle}
          onOpenAdvanced={openFilterSheet}
          onClearFilters={clearAllFilters}
          emptyTitle={categoryMeta.emptyTitle}
          emptyDescription={categoryMeta.emptyDescription}
          emptyIcon={categoryMeta.icon}
          yearsLabel={t('walker.years')}
          fromLabel={t('from')}
        />

        <AnimatePresence>
          {selectedProvider && (
            <DesktopSelectedProviderCard
              provider={selectedProvider}
              onClose={() => setSelectedProvider(null)}
              onWalkerClick={onWalkerClick}
            />
